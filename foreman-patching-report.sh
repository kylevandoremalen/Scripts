...
echo "Enter the environment to generate a report for (qa, stage, prod): "
read enviro
...
if [[ $enviro != "prod" ]] && [[ $enviro != "stage" ]] && [[ $enviro != "qa" ]];
then
        echo " "
        echo "------------------"
        echo "That is not a valid option. Please choose between the following environments:"
        echo "prod"
        echo "stage"
        echo "qa"
        exit 1
...
echo "Enter email address to send results to: "
read email
